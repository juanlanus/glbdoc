      ...
      *  db2 "create table TEST.SCOREID ( TIPOID smallint not null,
      *  NUMID bigint not null, primary key (TIPOID, NUMID))"
      *  db2 load from /d/iccol/desarrollo/temporales/SCOREIDS of DEL 
           exec sql
               declare SCORES cursor for 
               select SCORE.TIPOID, SCORE.NUMID, SCORE.FECHA,
               SCORE.SCORE, SCORE.EXCLUSION, SCORE.SEGMENTO
               from REGISTRO.SCORE, TEST.SCOREID
               on SCORE.TIPOID = SCOREID.TIPOID and SCORE.NUMID = SCOREID.NUMID
               order by SCORE.TIPOID, SCORE.NUMID, SCORE.FECHA
